%This script is for single parameter Kocks-Mecking model . The script is
%based on Kocks-Mecking equation dp/de=M(K+K1*sqrt(p)-K2*p). Here, 'p' is
% forest dislocation density. 'M' is Taylor factor. K1 is related to
% dislocation accumulation and K2 is related to dynamic recovery. K is
% related to dislocation storage due to dispersoids. 
...
total_plastic_strain=0.194;

%define the strain increment for performing the simulations
h=0.002; %a small value should always be used for accurate numerical integartion to be performed later
...
%strength
 ...
 k_1=2.0E8;         %Initial guess of K1
 k_2=15;             %Initial guess of K2 
 k=3E16;            %k=1/(burgers vector*grain size)
 ...
 yield=92;          %yield stength of the material
 stress(dynamic)=yield+orientationfactor*alpha*G*b*sqrt(Rho(dynamic));
 
while dynamic <=total_steps
    
              y_1= (k+k_1*sqrt(Rho(dynamic))-k_2*Rho(dynamic))*orientationfactor*h;
              
              y_2= (k+k_1*sqrt(Rho(dynamic)+0.5*y_1)-k_2*(Rho(dynamic)+0.5*y_1))*orientationfactor*h;
              
              y_3= (k+k_1*sqrt(Rho(dynamic)+0.5*y_2)-k_2*(Rho(dynamic)+0.5*y_2))*orientationfactor*h;
              
              y_4= (k+k_1*sqrt(Rho(dynamic)+y_3)-k_2*(Rho(dynamic)+y_3))*orientationfactor*h;
