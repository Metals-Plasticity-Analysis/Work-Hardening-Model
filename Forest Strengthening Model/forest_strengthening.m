...
%based on Kocks-Mecking equation dp/de=M(K1*sqrt(p)-K2*p). Here, 'p' is
% forest dislocation density. 'M' is Taylor factor. K1 is related to
% dislocation accumulation and K2 is related to dynamic recovery
...
%strength
 ...
 k_1=1.5E8;         %Initial guess of K1
 k_2=5;             %Initial guess of K2 
...
while dynamic <=total_steps
              ...
              y_1= (k_1*sqrt(Rho(dynamic))-k_2*Rho(dynamic))*orientationfactor*h;
              
              y_2= (k_1*sqrt(Rho(dynamic)+0.5*y_1)-k_2*(Rho(dynamic)+0.5*y_1))*orientationfactor*h;
              
              y_3= (k_1*sqrt(Rho(dynamic)+0.5*y_2)-k_2*(Rho(dynamic)+0.5*y_2))*orientationfactor*h;
              
              y_4= (k_1*sqrt(Rho(dynamic)+y_3)-k_2*(Rho(dynamic)+y_3))*orientationfactor*h;
